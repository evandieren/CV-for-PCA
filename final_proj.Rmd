...
plot(1:p, log(colMeans(errors[[1]])), xlab="Rank r", ylab="Error in logscale", main=TeX(paste0(method_str," error on $D_",dataset_number,"$ w.r.t noise"), bold=T),type="b",cex.main=1, cex.lab=1.5,cex.axis=1.4,lwd = 2,ylim = c(ymin,ymax),col=color_p[1])
...
Fixing our data set to be $D_0$ and applying our CV Methods, we get very different result for every single one of them. The error of the [Naïve Approach] decreases continuously and achieves its best estimation when allowing the method to make use of all $8$ variables.

The Wrong PCA Method Improved method however reacts negatively after surpassing the rank $r=3$ of our truncated data set. The slight perturbation show that the estimation usually get slightly worse. Overall this would lead the Wrong PCA Improved Method to favor rank $3$ as estimate for the rank of our modified data set. This behavior stays the same after having added high noise or low and differing noise. In general we can still see that the Wrong PCA has difficulties to extract the exact amount of variables, especially when the added noise to the data set is constantly high.

The [Missing data approach] differs in its overall behavior from the previous to methods. While the error only decreases slightly in the first and the second variable, it drops noticably for the third one (in our Simulation Study espcially for low and differing noise). Exceeding rank $3$ we observe a strong increase in the error and accordingly its estimation of the data set. In this case we can conclude the this approach also finds the correct dimension, but we have to be careful of the amount of noise added as this challenges the algorithm quite a bit.
...
Now we want to examine how our methods work, when moving from the identity matrix as covariance to a random matrix.
...
par(mfrow=c(2,2))
plot_dataset_together_log("Wrong PCA",1)
plot_dataset_together_log("Wrong PCA Improved",1)
plot_dataset_together_log("Missing Data",1)
plot_dataset_together_log("Matrix Completion",1)
```

The [Naive Approach] seems to keep the same shape and error size as on $D_0$. The Wrong PCA Improved method slightly changes as it still drops in general within the first three variables, but stabilizes a bit more afterwards. Again we see how the total noise added accounts for the different stabilization error. Interestingly, the Missing Data Approach  and the Matrix Completion Method aren't able to return more precise estimation over the amount of variables and actually increase in error size. Both have kind of the same shape and error size, whereas the latter one seems to be a smoother version of the first one. Also the error curve sticks closely together for the uniform noise in the Missing Data Approach 
...
par(mfrow=c(2,2))
plot_dataset_together_log("Wrong PCA",2)
plot_dataset_together_log("Wrong PCA Improved",2)
plot_dataset_together_log("Missing Data",2)
plot_dataset_together_log("Matrix Completion",2)
```
...
```{r NoiseMissing1, echo=FALSE,out.width="80%", fig.align = 'center', fig.width=12,fig.height=4,fig.cap = "KDE Approach for base 1 dataset with different noises as indicated by Noise°1,2 and 3"}
plot_three("KDE Approach",1)
```

```{r NoiseMissing2, echo=FALSE,out.width="80%", fig.align = 'center', fig.width=12,fig.height=4,fig.cap = "KDE Approach for base 2 dataset with different noises as indicated by Noise°1,2 and 3"}
plot_three("KDE Approach",2)
```

We will analyze the error of the [KDE modified approach] for all the different data set together as it behaves mostly independent of them and thus pretty similar. In general we see that for every variable until the third one it drops very strongly. Having reached rank $3$ the quality of its estimation seem to stay the same and correspondingly the error. In case of the second data set we observe the same behaviour but instead of stabilizing after rank $3$ it does that after reaching variable $2$ by construction of our data set.



## Comparison with respect to initial covariance matrix 

```{r Datasets, echo=FALSE,out.width="80%", fig.align = 'center', fig.width=12,fig.height=8,fig.cap = "Comparison of CV methods on different dataset for Noise n°1 (high noise)"}
...
```{r Dataset2, echo=FALSE,out.width="80%", fig.align = 'center', fig.width=12,fig.height=8,fig.cap = "Comparison of CV methods on different dataset for Noise n°2 (low noise)"}

par(mfrow=c(2,2))
plot_wrt_covariance("Wrong PCA",2)
plot_wrt_covariance("Wrong PCA Improved",2)
plot_wrt_covariance("Missing Data",2)
plot_wrt_covariance("Matrix Completion",2)

```

COMMENTS

```{r Dataset3, echo=FALSE,out.width="80%", fig.align = 'center', fig.width=12,fig.height=8,fig.cap = "Comparison of CV methods on different dataset for Noise n°3 (differing noise)"}
